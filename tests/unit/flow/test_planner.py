...
@pytest.mark.asyncio
async def test_planner_fallback_respects_resource_type_hints():
    state = base_state(FakeLLM(should_fail=True))
    state["resource_type_hints"] = ["doc", "mindmap", "reading"]

    result = await planner_node(state)

    assert [item["type"] for item in result["plan"]] == ["doc", "mindmap", "reading"]


@pytest.mark.asyncio
async def test_planner_llm_plan_is_aligned_to_resource_type_hints():
    llm = FakeLLM(
        {
            "tasks": [
                ...
                {"type": "video", "concept_ids": ["c3"], "difficulty": 0.6, "style_hint": "active", "constraints": []},
            ]
        }
    )
    state = base_state(llm)
    state["resource_type_hints"] = ["doc", "reading"]

    result = await planner_node(state)

    assert [item["type"] for item in result["plan"]] == ["doc", "reading"]


@pytest.mark.asyncio
async def test_planner_can_disable_llm_planner(monkeypatch):
    monkeypatch.setenv("ENABLE_LLM_PLANNER", "false")
    get_settings.cache_clear()
    llm = FakeLLM(
        {
            "tasks": [
                {"type": "video", "concept_ids": ["c1"], "difficulty": 0.4, "style_hint": "active", "constraints": []},
            ]
        }
    )
    state = base_state(llm)
...
@pytest.mark.asyncio
async def test_planner_eval_force_collab_mode_overrides_llm(monkeypatch):
    monkeypatch.setenv("EVAL_FORCE_COLLAB_MODE", "central")
    get_settings.cache_clear()
    llm = FakeLLM(
        {
            "tasks": [
