...
@pytest.mark.asyncio
async def test_recall_semantic_records_memory_reuse_metrics(monkeypatch):
    """召回命中后记录记忆复用指标，为自进化飞轮提供可观测证据。"""
    from reflexlearn.observability.metrics import export_metrics

    monkeypatch.setattr(emb, "embed_query", lambda q: [0.1] * emb.EMBED_DIM)
    fake = _FakeQdrant(hits=[_Point({"user_id": "u1", "task_type": "quiz", "cause": "复用历史经验"})])

    await recall_reflections(qdrant=fake, task_type="", query="线性回归", acl={"user_id": "u1"})

    body = export_metrics().decode("utf-8")
