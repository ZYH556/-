...
@pytest.mark.asyncio
async def test_profile_roundtrip_is_scoped_by_user_and_tenant(monkeypatch):
    fake = _FakeRedis()
    _patch(monkeypatch, fake)
    ...
    ok = await session_store.save_profile("u1", tenant_id="t1", profile=profile)
    assert ok is True

    assert await session_store.load_profile("u1", tenant_id="t1") == profile
    assert await session_store.load_profile("u1", tenant_id="t2") == {}
    assert await session_store.load_profile("u2", tenant_id="t1") == {}


@pytest.mark.asyncio
async def test_profile_store_degrades_on_redis_down(monkeypatch):
    _patch(monkeypatch, _FakeRedis(fail=True))

    assert await session_store.load_profile("u1", tenant_id="t1") == {}
    assert await session_store.save_profile("u1", tenant_id="t1", profile={"goal": "x"}) is False


@pytest.mark.asyncio
async def test_profile_store_corrupt_json_returns_empty(monkeypatch):
    fake = _FakeRedis()
    fake.store["profile:t1:u1"] = "{not valid json"
    _patch(monkeypatch, fake)
