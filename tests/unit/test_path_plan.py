...
# ———————————————————— 集成：assemble → path_plan → END ————————————————————

class _IntegrationLLM:
    """planner→2 任务(doc+quiz, central)；path_plan→抛错走规则排序(可预测教学序)；
    其余(生成/质检)→长文本以通过基于长度的质量校验。"""

    async def complete(self, messages, *, task_type="generation", schema=None, temperature=0.7):
        sys = messages[0]["content"] if messages else ""
        if "路径规划" in sys:  # 仅 PathPlanSkill 的 system prompt 含此词
                ...
                text=json.dumps(
                    {
                        "tasks": [
                            ...
                            {"type": "quiz", "concept_ids": ["c2"], "difficulty": 0.6, "style_hint": "active", "constraints": []},
                        ]
                    },
                    ensure_ascii=False,
                )
        ...
        return Completion(
            text=(
                # 必须 > 50 字符：quality_check._rule_check 用 len(content) > 50 判定通过
                "这是一段足够长的机器学习教学内容，用于通过基于长度（>50 字）的质量校验规则，"
...
def _e2e_initial() -> dict:
    return {
        ...
        "learner_profile": {},
        "learning_goal": "线性回归基础",  # 不含 pipeline/debate 关键词 → central
        "collab_mode": "central",
        "plan": [],
        "completed": [],
        "reflections": [],
...
async def test_build_graph_assemble_to_path_plan_to_end():
    ...
    assert len(path) >= 2
    assert [s["sequence"] for s in path] == list(range(1, len(path) + 1))  # 连续递增
    assert all(s.get("objective") for s in path)                          # 规则路径含学习目标

    # 零回归：resource_bundle 仍在 assemble 帧产出
...
async def test_path_plan_enriches_over_assemble_simple_path():
    ...
    assert all("objective" not in p for p in assemble_path)  # assemble 简单路径
    assert pp_path[0].get("objective")                       # path_plan 更丰富（覆盖语义）
